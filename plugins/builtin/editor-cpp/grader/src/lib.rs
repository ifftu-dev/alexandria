//! C/C++ grader/runner ABI. The source is interpreted by the bundled JSCPP
//! (a C/C++ interpreter in JavaScript) running inside `boa-runner-core`'s Boa
//! engine. The engine, scoring, and wire formats all live in `boa-runner-core`.
//!
...
/// Bundled JSCPP (IIFE exposing `AlexJSCPP.JSCPP.run`). Produced by
/// `editor-shared/build.sh` from `editor-shared/cm6-build`.
const JSCPP: &str = include_str!("jscpp.js");
...
pub extern "C" fn alex_grade(ptr: i32, len: i32) -> i64 {
    ...
    pack(grade(&bytes, Lang::Cpp(JSCPP)))
}

#[no_mangle]
pub extern "C" fn alex_run(ptr: i32, len: i32) -> i64 {
    let bytes = unsafe { read_input(ptr, len) };
    pack(run(&bytes, Lang::Cpp(JSCPP)))
}
