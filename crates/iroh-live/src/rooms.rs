...
/// How often each peer re-announces its presence (also refreshes liveness so
/// entries do not expire while the peer is still here).
...
struct Actor {
    ...
    /// Sender for our own presence announcements on the room gossip topic.
    gossip_tx: GossipSender,
    /// Receiver for peers' announcements. Taken out in `run`.
    gossip_rx: Option<GossipReceiver>,
    /// Last time we heard a presence announcement from each peer, for expiry.
    peer_last_seen: HashMap<EndpointId, Instant>,
}
...
impl Actor {
    async fn new(
        ...
        Ok(Self {
            ...
            peer_last_seen: Default::default(),
        })
    }
    ...
    pub async fn run(mut self, mut inbox: mpsc::Receiver<ApiMessage>) {
        ...
        // Re-announce our own presence periodically so new joiners learn our
        // broadcasts and existing peers refresh our liveness.
        let mut presence_interval = tokio::time::interval(PRESENCE_INTERVAL);
                ...
                Some(event) = updates.next() => {
                    match event {
                        Ok(Event::Received(msg)) => {
                            match postcard::from_bytes::<PeerAnnounce>(&msg.content) {
                                Ok(ann) => self.handle_gossip_update(ann).await,
                                Err(err) => debug!("undecodable presence announce: {err:#}"),
                            }
                        }
                        Ok(_) => {} // NeighborUp/Down/Lagged — not presence
                        Err(err) => warn!("gossip presence stream error: {err:#}"),
                    }
                }
    ...
    async fn handle_gossip_update(&mut self, ann: PeerAnnounce) {
        let PeerAnnounce {
            from: remote,
            broadcasts,
        } = ann;
        ...
        // Refresh liveness for expiry.
        self.peer_last_seen.insert(remote, Instant::now());
    ...
    async fn update_kv(&self) {
        ...
        let bytes = match postcard::to_stdvec(&ann) {
            Ok(b) => b,
            Err(err) => {
                warn!("failed to encode presence announce: {err:#}");
                return;
            }
        };
        if let Err(err) = self.gossip_tx.broadcast(bytes.into()).await {
            warn!("failed to broadcast presence announce: {err:#}");
        }
    }

    /// Drop peers whose presence has not been refreshed within the horizon.
    fn prune_expired_peers(&mut self) {
        let now = Instant::now();
        let expired: Vec<EndpointId> = self
            .peer_last_seen
            .iter()
            .filter(|(_, seen)| now.duration_since(**seen) > PRESENCE_HORIZON)
            .map(|(id, _)| *id)
            .collect();
        for id in expired {
            self.peer_last_seen.remove(&id);
            self.known_remote_broadcasts.remove(&id);
        }
    }
}
