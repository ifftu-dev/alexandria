...
impl AudioEncoderInner for PureOpusEncoder {
    ...
    fn config(&self) -> AudioConfig {
        // hang 0.19 `AudioConfig` is `#[non_exhaustive]` — build via `new`
        // (codec, sample_rate, channel_count) then set optional fields.
        // The pure opus encoder needs no extradata: the decoder initializes
        // from the config fields alone (matches the ffmpeg decoder, which
        ...
        config.bitrate = Some(self.bitrate);
        config
    }
    ...
    fn pop_packet(&mut self) -> Result<Option<Frame>> {
        ...
        // Every opus frame is independently decodable → keyframe. moq-mux turns
        // keyframe frames into fresh moq-net group boundaries.
        let frame = Frame {
        ...
        trace!("pop_packet: {} bytes", frame.payload.len());
        Ok(Some(frame))
    }
}
