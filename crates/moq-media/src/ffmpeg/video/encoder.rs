...
impl H264Encoder {
    ...
    pub fn video_config(&self) -> Result<hang::catalog::VideoConfig> {
        // hang 0.19 `VideoConfig` is `#[non_exhaustive]` — use the builder.
        ...
        config.description = Some(self.avcc_description()?.to_vec().into());
        config.coded_width = Some(self.opts.width);
        config.coded_height = Some(self.opts.height);
        config.bitrate = Some(self.opts.bitrate);
        config.framerate = Some(self.opts.framerate as f64);
    ...
    pub fn receive_packet(&mut self) -> Result<Poll<Option<moq_mux::container::Frame>>> {
        loop {
            ...
            match self.encoder.receive_packet(&mut packet) {
                Ok(()) => {
                    ...
                    if is_keyframe
                        && self.sps_pps.is_none()
                        && let Some(ed) = self.encoder.extradata()
                    {
                    ...
                    if is_keyframe
                        && let Some(ref sps_pps) = self.sps_pps
                        && !sps_pps.is_empty()
                    {
                        let mut combined = Vec::with_capacity(sps_pps.len() + payload.len());
                        combined.extend_from_slice(sps_pps);
                        combined.extend_from_slice(&payload);
                        payload = combined;
                    }

                    let hang_frame = moq_mux::container::Frame {
...
impl av::VideoEncoderInner for H264Encoder {
    ...
    fn pop_packet(&mut self) -> anyhow::Result<Option<moq_mux::container::Frame>> {
