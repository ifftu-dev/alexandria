...
impl VtEncoder {
    fn create(width: u32, height: u32, fps: f64, bitrate: u32) -> Result<Self> {
        ...
        // Build initial VideoConfig (description filled when first keyframe arrives).
        ...
        config.framerate = Some(fps);
        config.bitrate = Some(bitrate as u64);
...
impl VideoEncoderInner for VtEncoder {
    ...
    fn pop_packet(&mut self) -> Result<Option<moq_mux::container::Frame>> {
        let mut q = self
            .output
            .lock()
        ...
        // moq-mux frames carry `bytes::Bytes` (hang 0.19 dropped the BufList
        // payload). VideoToolbox hands us a single contiguous chunk.
        Ok(Some(moq_mux::container::Frame {
            payload: pkt.data,
