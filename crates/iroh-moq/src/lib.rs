...
use moq_lite::{
    BroadcastConsumer, BroadcastProducer, Client, Origin, OriginConsumer, OriginProducer, Server,
};
...
use tracing::{Instrument, error_span, info, instrument};
use web_transport_iroh::SessionError;

// moq-lite/moq-net "lite" ALPN. In moq-net 0.17 the per-version ALPN constants
// are crate-private (`ALPN_LITE = "moql"`); both ends here are our own app, so
// we pin the lite ALPN directly for the QUIC handshake and let moq-net's SETUP
// negotiate the exact version after the connection is up.
pub const ALPN: &[u8] = b"moql";
...
pub struct MoqSession {
    ...
    /// Broadcasts we serve to the peer. We hold the producer; the moq client/
    /// server was handed `publish.consume()` and forwards what we publish here.
    publish: OriginProducer,
    /// Broadcasts the peer announces to us. The moq client/server writes into
    /// the paired producer; this consumer reads the announcements.
    subscribe: OriginConsumer,
    /// The moq-net session. Kept alive: in moq-net 0.17 it is `Arc`-backed and
    /// drives the connection pump, so dropping it would tear the session down.
    _moq: moq_lite::Session,
}
...
impl MoqSession {
    ...
    pub async fn session_connect(wt_session: web_transport_iroh::Session) -> Result<Self, Error> {
        ...
        let subscribe = Origin::random().produce();
        let subscribe_consumer = subscribe.consume();
        // moq-net's Client reads broadcasts from the publish consumer and writes
        // the peer's announcements into the subscribe producer (moved in).
        let moq = Client::new()
            .with_publish(publish.consume())
            .with_consume(subscribe)
    ...
    pub fn remote_id(&self) -> EndpointId {
        self.wt_session.conn().remote_id()
    }
    ...
    pub async fn subscribe(&mut self, name: &str) -> Result<BroadcastConsumer, SubscribeError> {
        if self.wt_session.close_reason().is_some() {
            return Err(e!(SubscribeError::Closed));
        }
        // `announced_broadcast` blocks until the exact path is announced (or the
        // consumer closes), racing the session close so a dead peer errors out.
        tokio::select! {
            broadcast = self.subscribe.announced_broadcast(name) => {
                broadcast.ok_or_else(|| e!(SubscribeError::NotAnnounced))
            }
            reason = self.wt_session.closed() => Err(reason.into()),
        }
    }
...
impl Actor {
    ...
    fn handle_publish_broadcast(&mut self, name: BroadcastName, producer: BroadcastProducer) {
        ...
        // Hold the consumer inside the future: moq-net's `closed()` borrows
        // `&self`, so the consumer must outlive the await.
