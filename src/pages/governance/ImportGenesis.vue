...
const locator = ref<GovernanceGenesisLocator | null>(null)
...
const canPin = computed(
  () => retrieved.value !== null && confirmation.value === retrieved.value.preview.dao_id,
)
...
async function reviewLocator(): Promise<void> {
  ...
  try {
    locator.value = await invoke<GovernanceGenesisLocator>(
      'governance_preview_genesis_locator',
      { locatorUri: locatorInput.value },
    )
  } catch (reason) {
...
async function retrieveGenesis(): Promise<void> {
  ...
  try {
    retrieved.value = await invoke<RetrievedGenesisPreview>('governance_retrieve_genesis', {
      locatorUri: locatorInput.value,
    })
  } catch (reason) {
    error.value = message(reason)
...
async function pinGenesis(): Promise<void> {
  if (!retrieved.value || !canPin.value) return
  error.value = ''
  success.value = ''
  pinning.value = true
  try {
    const result = await invoke<PinGenesisResponse>('governance_pin_genesis', {
      genesisJson: retrieved.value.genesis_json,
...
<template>
  <div class="mx-auto max-w-4xl space-y-6">
    ...
    <section v-if="locator" class="rounded-xl bg-card p-5 shadow-sm">
      ...
      <dl class="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <dd class="mt-1 break-all font-mono text-xs text-foreground">{{ locator.dao_id }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.contentHash') }}</dt>
          <dd class="mt-1 break-all font-mono text-xs text-foreground">{{ locator.content_hash }}</dd>
        </div>
      </dl>
      ...
      <ul class="mt-2 space-y-2">
        <li v-for="source in locator.locations" :key="source" class="break-all rounded bg-background p-2 font-mono text-xs text-foreground">
          {{ source }}
        </li>
      </ul>
      <details class="mt-4 rounded border border-border p-3">
        ...
        <QrCodeDisplay class="mt-3" :value="locatorInput" :size="240" />
      </details>
    ...
    <section v-if="retrieved" class="rounded-xl bg-card p-5 shadow-sm">
      <h2 class="text-base font-semibold text-foreground">
      ...
      <dl class="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.name }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.scope') }}</dt>
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.scope_type }} / {{ retrieved.preview.scope_id }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.daoId') }}</dt>
          <dd class="mt-1 break-all font-mono text-xs text-foreground">{{ retrieved.preview.dao_id }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.rulesHash') }}</dt>
          <dd class="mt-1 break-all font-mono text-xs text-foreground">{{ retrieved.preview.rules_hash }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.versions') }}</dt>
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.protocol_version }} / {{ retrieved.preview.rules_version }}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.minimum_turnout_count }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.approvalRule') }}</dt>
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.proposal_approval_numerator }} / {{ retrieved.preview.proposal_approval_denominator }}</dd>
        </div>
        <div>
          ...
          <dd class="mt-1 break-all text-sm text-foreground">
            {{ retrieved.preview.cometbft_chain_id }} / {{ retrieved.preview.initial_epoch }} / {{ retrieved.preview.initial_height }}
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd class="mt-1 text-sm text-foreground">{{ retrieved.preview.qualification_policy_version }}</dd>
        </div>
        <div>
          <dt class="text-xs text-muted-foreground">{{ $t('governanceGenesisImport.resolvedFrom') }}</dt>
          <dd class="mt-1 break-all font-mono text-xs text-foreground">{{ retrieved.resolved_from }}</dd>
        </div>
      </dl>
      ...
      <div class="mt-5 grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <ul class="mt-2 space-y-1">
            <li v-for="issuer in retrieved.preview.accepted_issuers" :key="issuer" class="break-all font-mono text-xs text-muted-foreground">{{ issuer }}</li>
          </ul>
        </div>
        <div>
          <h3 class="text-sm font-medium text-foreground">{{ $t('governanceGenesisImport.evidence') }}</h3>
          <ul class="mt-2 space-y-1">
            <li v-for="item in retrieved.preview.accepted_assessment_evidence" :key="item" class="break-all font-mono text-xs text-muted-foreground">{{ item }}</li>
          </ul>
        </div>
      </div>
      ...
      <div class="mt-2 space-y-3">
        <details v-for="member in retrieved.preview.members" :key="member.member_id" class="rounded border border-border p-3">
          <summary class="cursor-pointer text-sm font-medium text-foreground">{{ member.member_id }}</summary>
          <dl class="mt-3 space-y-2 font-mono text-xs text-muted-foreground">
            <div><dt>{{ $t('governanceGenesisImport.identityKey') }}</dt><dd class="break-all">{{ member.identity_public_key_hex }}</dd></div>
            <div><dt>{{ $t('governanceGenesisImport.consensusKey') }}</dt><dd class="break-all">{{ member.consensus_public_key_hex }}</dd></div>
            <div><dt>{{ $t('governanceGenesisImport.governanceKey') }}</dt><dd class="break-all">{{ member.governance_public_key_hex }}</dd></div>
          </dl>
        </details>
      </div>
