  ...
  <div>
    ...
    <div class="mb-8 flex items-start justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-[rgb(var(--color-foreground))] sm:text-3xl">Governance</h1>
    ...
    <div v-if="loading" class="space-y-6">
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div v-for="i in 3" :key="i" class="animate-pulse rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6">
          ...
          <div class="h-8 w-10 rounded bg-[rgb(var(--color-muted-foreground)/0.2)]" />
        </div>
      </div>
      <div class="h-10 w-72 animate-pulse rounded-lg bg-[rgb(var(--color-muted-foreground)/0.1)]" />
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div v-for="i in 4" :key="i" class="animate-pulse rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-5">
          <div class="flex items-start gap-3">
            <div class="h-10 w-10 rounded-lg bg-[rgb(var(--color-muted-foreground)/0.15)]" />
            <div class="flex-1 space-y-2">
              <div class="h-4 w-40 rounded bg-[rgb(var(--color-muted-foreground)/0.15)]" />
    ...
    <template v-else>
      ...
      <div class="mb-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6">
          <p class="text-sm text-[rgb(var(--color-muted-foreground))]">Total DAOs</p>
          <p class="mt-2 text-3xl font-bold text-[rgb(var(--color-foreground))]">{{ totalDaos }}</p>
        </div>
        <div class="rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6">
          <p class="text-sm text-[rgb(var(--color-muted-foreground))]">Active</p>
          <p class="mt-2 text-3xl font-bold text-emerald-500">{{ activeDaos }}</p>
        </div>
        <div class="rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6">
          <p class="text-sm text-[rgb(var(--color-muted-foreground))]">Total Committee Seats</p>
      ...
      <div v-if="filtered.length === 0" class="py-16 text-center">
        ...
        <h3 class="text-lg font-semibold text-[rgb(var(--color-foreground))]">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <router-link
          ...
          class="group rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-5 transition-all hover:border-amber-500/30 hover:shadow-sm"
        >
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="text-sm font-semibold text-[rgb(var(--color-foreground))] truncate group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                  {{ dao.name }}
                </h3>
                <StatusBadge :status="dao.status" />
              </div>
              <p v-if="dao.description" class="text-xs text-[rgb(var(--color-muted-foreground))] line-clamp-2 mb-2">
                {{ dao.description }}
              </p>
