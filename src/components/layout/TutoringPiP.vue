    ...
    <div class="overflow-hidden rounded-2xl border border-border/80 bg-card/96 shadow-2xl ring-1 ring-black/5 backdrop-blur-md">
          ...
          <div class="flex items-end justify-between gap-3">
            <div class="min-w-0">
              ...
              <p class="mt-0.5 text-xs text-white/75">
                {{ $t('tutoring.pip.connected', { connected: connectedPeerCount, total: peerCount }) }}
              </p>
            </div>
            <div class="flex items-center gap-2 rounded-full bg-black/55 px-2.5 py-1 text-[0.7rem] font-medium text-white backdrop-blur-sm">
              <span>{{ audioEnabled ? $t('tutoring.pip.micOn') : $t('tutoring.pip.micOff') }}</span>
      ...
      <div class="flex items-center justify-between gap-2 border-t border-border/70 px-3 py-2.5">
        ...
        >
          ...
          <span>{{ audioEnabled ? $t('tutoring.controls.mute') : $t('tutoring.controls.unmute') }}</span>
        </button>
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            @click="openSession"
          >
            <span>{{ $t('tutoring.controls.open') }}</span>
          </button>
          <button
            ...
            @click="handleLeave"
          >
            <span>{{ $t('tutoring.controls.leave') }}</span>
          </button>
        </div>
      </div>
    </div>
