  ...
  <div class="video-player">
    ...
    <div v-else-if="videoUrl" class="space-y-3">
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="text-xs text-[rgb(var(--color-muted-foreground))] font-mono whitespace-nowrap">
          {{ formatTime(progress) }} / {{ formatTime(duration) }}
        </span>
      </div>

      <div v-if="hasEnded" class="flex items-center gap-2 text-sm text-[rgb(var(--color-success))]">
    ...
    <div v-else class="flex flex-col items-center justify-center rounded-lg border border-dashed border-[rgb(var(--color-border))] bg-[rgb(var(--color-muted)/0.3)] aspect-video">
      <svg class="w-12 h-12 text-[rgb(var(--color-muted-foreground)/0.5)] mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" />
      </svg>
      <p class="text-sm font-medium text-[rgb(var(--color-muted-foreground))]">{{ title ?? 'Video' }}</p>
      <p class="text-xs text-[rgb(var(--color-muted-foreground)/0.6)] mt-1">Video content not yet available on this node</p>
    </div>
  </div>
</template>
