...
///
/// Governance DAOs are deliberately not created here. A taxonomy scope may
/// acquire a DAO only through the separately verified founding-roster flow.
...
mod tests {
        ...
        TaxonomyChanges {
            ...
            skills: vec![
                TaxonomySkill {
                    ...
                    bloom_level: crate::domain::bloom::BloomLevel::Apply,
                },
                TaxonomySkill {
    ...
    fn publish_applies_changes() {
        ...
        let auto_created_daos: i64 = db
            .conn()
            .query_row(
                "SELECT COUNT(*) FROM governance_daos WHERE scope_id IN ('cs', 'algorithms')",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(
            auto_created_daos, 0,
            "taxonomy publication must not activate governance authority"
        );
    }
    ...
    fn version_chain_maintained() {
        ...
        let changes2 = TaxonomyChanges {
            ...
            skills: vec![TaxonomySkill {
                ...
                description: None,
                subject_id: "algorithms".into(),
                bloom_level: crate::domain::bloom::BloomLevel::Analyze,
            }],
            prerequisites: vec![],
            removed_prerequisites: vec![],
        };
