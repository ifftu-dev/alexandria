...
use crate::commands::pairing::NodeHandle;
...
pub async fn guardian_list_links(
    ...
    state
        ...
        .execute(
            ...
            state.profile_lease(),
            "guardian.links.list",
            move |db| list_links(db.conn()),
        )
        .await
}

fn list_links(conn: &rusqlite::Connection) -> Result<Vec<GuardianLinkInfo>, String> {
    let mut stmt = conn
        .prepare(
...
fn row_to_link(row: &rusqlite::Row<'_>) -> rusqlite::Result<GuardianLinkInfo> {
    Ok(GuardianLinkInfo {
        id: row.get(0)?,
        side: row.get(1)?,
        peer_did: row.get(2)?,
...
pub async fn guardian_create_invite(state: State<'_, AppState>) -> Result<String, String> {
    ...
    let (child_did, child_stake_address, display_name) = state
        .db_executor
        .execute(
            DatabaseWorkload::Learner,
            state.profile_lease(),
            "guardian.invite.identity",
            move |db| {
                let conn = db.conn();
                let did: String = conn
                    .query_row(
                ...
                let (stake, name): (String, Option<String>) = conn
                    ...
                    .map_err(|error| format!("no identity: {error}"))?;
                Ok((did, stake, name))
            },
        )
        .await?;
    ...
    state
        ...
        .execute(
            DatabaseWorkload::Learner,
            state.profile_lease(),
            "guardian.invite.persist",
            move |db| {
                proto::record_pending_invite(db.conn(), &code_hash, &shared_key, INVITE_TTL_SECS)
            },
        )
        .await?;
    Ok(code)
...
pub async fn guardian_accept_invite(
    ...
    let code_hash_for_db = code_hash.clone();
    let issuer_did_for_db = issuer_did.clone();

    let (link_id, guardian_vc_json, guardian_did, guardian_stake, guardian_name) = state
        .db_executor
        .execute(
            DatabaseWorkload::Learner,
            state.profile_lease(),
            ...
            move |db| {
                ...
                crate::db::with_transaction(conn, || {
                    let req = crate::commands::credentials::IssueCredentialRequest {
                        ...
                        subject: crate::crypto::did::Did(invite_for_db.child_did.clone()),
                        claim: Claim::Role(RoleClaim {
                            role: "guardian".to_string(),
                            scope: Some(issuer_did_for_db.as_str().to_string()),
                        }),
                        evidence_refs: vec![],
                    ...
                    let link_id = crate::crypto::hash::entity_id(&[
                        issuer_did_for_db.as_str(),
                        &invite_for_db.child_did,
                        &now,
                    ]);
                    let (stake, name): (String, Option<String>) = conn
                        .query_row(
             ...
             VALUES (?1, 'guardian', ?2, ?3, ?4, ?5, ?6, 'pending', ?7, ?8)",
                        rusqlite::params![
                            link_id,
                            invite_for_db.child_did,
                            invite_for_db.child_stake_address,
                            invite_for_db.child_peer_id,
                            invite_for_db.display_name,
                            &invite_for_db.shared_key[..],
                            vc_id,
                            code_hash_for_db
                        ],
                    )
                    ...
                    if !invite_for_db.addresses.is_empty() {
                        let addrs_json =
                            serde_json::to_string(&invite_for_db.addresses).map_err(|error| {
                                format!("serialize guardian peer addresses: {error}")
                            })?;
                        conn.execute(
                            "INSERT INTO peers (peer_id, addresses, last_seen) \
                 VALUES (?1, ?2, datetime('now')) \
                 ON CONFLICT(peer_id) DO UPDATE SET addresses = ?2, last_seen = datetime('now')",
                            rusqlite::params![invite_for_db.child_peer_id, addrs_json],
                        )
                        .map_err(|e| e.to_string())?;
                    }
                    ...
                    Ok((
                        link_id,
                        vc_json,
                        issuer_did_for_db.as_str().to_string(),
                        stake,
                        name,
                    ))
                })
    ...
    get_link(&state.db_executor, state.profile_lease(), &link_id).await
}

#[allow(clippy::too_many_arguments)]
async fn send_link_request(
    db_executor: &DatabaseExecutor,
    lease: &crate::profile::scope::ProfileLease,
...
pub(crate) async fn guardian_sync_all(
    ...
    let links: Vec<SyncableLink> = db_executor
        .execute(workload, lease.clone(), "guardian.sync.links", move |db| {
            let mut stmt = db
                .conn()
                .prepare(
    ...
    for (link_id, side, link_status, key_blob, peer_id, _code_hash) in links {
        ...
        match (side.as_str(), link_status.as_str()) {
            ("ward", "active") => {
                if let Ok(rows) = push_to_guardian(
                    db_executor,
                    lease,
                    workload,
                    node_handle,
            ...
            ("guardian", "pending") => {
                if let Err(e) =
                    retry_pending_link(db_executor, lease, workload, node_handle, &link_id).await
                {
...
async fn push_to_guardian(
    ...
    let key_for_snapshot = *key;
    let sealed = db_executor
        .execute(workload, lease.clone(), "guardian.sync.build", move |db| {
            let snapshot = proto::build_activity_snapshot(db.conn())?;
            proto::seal(&key_for_snapshot, &snapshot)
        })
        .await?;
        ...
        GuardianResponse::Merged { rows } => {
            ...
            db_executor
                .execute(
                    workload,
                    lease.clone(),
                    "guardian.sync.mark-pushed",
                    move |db| {
                        db.conn()
                            .execute(
                                "UPDATE guardian_links SET last_sync_at = datetime('now') \
                                 WHERE id = ?1",
                                rusqlite::params![link_id],
                            )
                            .map_err(|error| error.to_string())?;
...
async fn pull_from_ward(
    ...
    match response {
        GuardianResponse::Sealed { sealed } => {
            ...
            let link_id = link_id.to_string();
            db_executor
                .execute(workload, lease.clone(), "guardian.sync.apply", move |db| {
                    proto::apply_activity_snapshot(db.conn(), &link_id, &payload)
                })
                .await
        }
...
async fn retry_pending_link(
    ...
    let link_id_for_read = link_id.to_string();
    let (invite, code_hash, guardian_vc_json, guardian_did, guardian_stake, guardian_name) =
        db_executor
            .execute(
                workload,
                lease.clone(),
                "guardian.link.retry-load",
                move |db| {
                    let conn = db.conn();
                    let (
                        peer_did,
                        ...
                        .query_row(
                            ...
                            rusqlite::params![link_id_for_read],
                            |row| {
                                Ok((
                                    row.get(0)?,
                                    row.get(1)?,
                    ...
                    let vc_json: String = conn
                        ...
                        .map_err(|error| format!("stored guardian VC missing: {error}"))?;
                    let guardian_did: String = conn
                        ...
                        .map_err(|error| error.to_string())?
                        .unwrap_or_default();
                    let (guardian_stake, guardian_name): (String, Option<String>) = conn
                        .query_row(
                            "SELECT stake_address, display_name FROM local_identity WHERE id = 1",
                            [],
                            |row| Ok((row.get(0)?, row.get(1)?)),
...
pub async fn guardian_revoke_link(
    ...
    let link_id_for_db = link_id.clone();
    let (side, key_blob, peer_id): (String, Vec<u8>, Option<String>) = state
        .db_executor
        .execute(
            DatabaseWorkload::Learner,
            state.profile_lease(),
        ...
        let row: (String, Vec<u8>, Option<String>) = conn
            .query_row(
                ...
                rusqlite::params![link_id_for_db],
