                    ...
                    if let Some(ref role) = role {
                        (
                            "SELECT id, actor_address, role, skill_id, proficiency_level, score, \
                             ...
                             FROM current_reputation_assertions WHERE role = ?1 ORDER BY updated_at DESC"
                                .to_string(),
                            vec![Box::new(role.clone())],
                        )
                    } else {
                        (
                            "SELECT id, actor_address, role, skill_id, proficiency_level, score, \
                             evidence_count, computation_spec, updated_at \
                             FROM current_reputation_assertions ORDER BY updated_at DESC"
                                .to_string(),
                            vec![],
                        )
                    };
