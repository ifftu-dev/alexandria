...
/// activation state. Both are required: everybody is a learner, so everybody
/// gives a birthdate, and an instructor or parent under 18 is gated exactly
...
fn resolve_account_fields(
    ...
    let roles = roles.ok_or_else(|| "Roles are required.".to_string())?;
    let roles = crate::domain::identity::normalize_roles(&roles)?;

    let birthdate = birthdate.ok_or_else(|| "A birthdate is required.".to_string())?;
    let birthdate = crate::domain::identity::validate_birthdate(&birthdate, today)?;

    let activation = if crate::domain::identity::is_minor(&birthdate, today) {
        "pending_guardian"
    } else {
        "active"
    };
...
#[tauri::command]
pub async fn get_profile_cleanup_required(state: State<'_, AppState>) -> Result<bool, String> {
...
pub async fn get_profile_session_token(
    ...
    Ok(state.profile_operations.session())
}
...
pub async fn create_profile(
    ...
    let operations = app.state::<AppState>().profile_operations.clone();
    ...
    operations
        ...
        }, async move { cleanup_app.state::<AppState>().stop_active_profile().await })
        .await
}
...
pub async fn unlock_profile(
    ...
    operations
        .activate(async move {
            ...
            if state.active_id().is_some() {
                return Err("lock the active profile before unlocking another one".to_string());
            }
...
#[tauri::command]
pub async fn lock_profile(app: AppHandle) -> Result<(), String> {
    crate::commands::diagnostics::force_exit(&app);
    let operations = app.state::<AppState>().profile_operations.clone();
    operations
        .lock(async move { app.state::<AppState>().stop_active_profile().await })
        .await
}
...
#[tauri::command]
pub async fn delete_profile(app: AppHandle, id: String, password: String) -> Result<(), String> {
    let operations = app.state::<AppState>().profile_operations.clone();
    operations
        .run(async move {
            let state = app.state::<AppState>();
