...
| T03 | In progress | `3479942`: action-to-policy matrix plus pinned canonical subject qualification policies and evaluator; `b1622ff`: fail-closed startup policy loading and one evaluator for local publish, inbound gossip, pending promotion and eligible fields (preprod pins no policies, so posting is refused); `e994a69`: derived skill states score only verified inputs, self-issued claims add no issuer independence, fingerprinted caches invalidate on input change or removal; `ff30a0b`: reputation rows and snapshot evidence on the same verified inputs with issuer-signed-only instructor credit; `755ce56`: local revocation checks are shown as pending when status evidence is missing and labelled as limited to status information held on the device; the talent-index reader revalidating cached states with verified Bloom levels is in review; closure against the required tests is next |
...
Derived skill states, reputation rows, snapshot evidence and the talent-index
reader now share verified scoring inputs, and the credential proof panel no
longer presents a local revocation check as current remote status. The next
step closes T03 against its required tests. The preprod profile pins no
