...
| T03 | Verified | `3479942`: action-to-policy matrix plus pinned canonical subject qualification policies and evaluator; `b1622ff`: fail-closed startup policy loading and one evaluator for local publish, inbound gossip, pending promotion and eligible fields (preprod pins no policies, so posting is refused); `e994a69`: derived skill states score only verified inputs, self-issued claims add no issuer independence, fingerprinted caches invalidate on input change or removal; `ff30a0b`: reputation rows and snapshot evidence on the same verified inputs with issuer-signed-only instructor credit; `755ce56`: local revocation checks are shown as pending when status evidence is missing and labelled as limited to status information held on the device; `864a07d`: talent-index candidates come from revalidated current-version states with Bloom levels from verified inputs; `cb3bda7`: public-derived issuer exclusion retired by migration 094, reputation rows fingerprinted and revalidated before every read, and its adversarial tests rewritten on signed fixtures. Every required test is covered; the preprod profile still pins no qualification policies, so opinion posting stays refused |
| T04 | Verified | `f67fcc1`: strict bounded JSON parser in the verifier; network profile, credential file/bundle/peer import, offline verification, guardian link and course document boundaries parse under explicit limits with exact-boundary and one-over tests; `095445d`: gossip envelopes, the payload schema step and peer exchange decode under explicit limits; `dfb23f7`: 24 exact-byte limit vectors that the Rust verifier and the independent Node verifier both pass; `72fe89f`: 19 exact-byte course-endorsement vectors and 3 threshold sets shared by the Rust verifier, the independent Node verifier and the app import, which now takes the pasted text unchanged; `22950b5`: signed profile documents bounded on resolve and publish. Remaining inbound boundaries are handed to H03 (device-sync and guardian payloads), N02 (username claims and receipts), N03 (fetch codecs), C02 (presentations and holder fetches) and N01 (the digest-pinned bootstrap snapshot). Verification awaits the cloud consumer passing the same vectors in C01 |
| D01 | Verified | Reference inventory complete. `5749155`: local elections, proposals, committee install, operator DAO bootstrap, the operator governance queue and its Cardano builders, encoders and operator key are deleted with their commands, pages, search domain, types and locale catalogs; `/community` opens genesis review and the governance gossip handler only rejects. `92074c6`: Sentinel prior proposal, ratification, library commands, runtime classifier replacement, kill switch, version blocklist, the Sentinel DAO card and the propose page are deleted with 13 commands; the paste classifier parses only its embedded model, holdout keeps the moved blob validators, and the prior gossip handler only rejects. `f646bec`: caller-declared taxonomy ratification (including the ungated `propose_taxonomy_change` proposal write) and goal-template and question-bank ratification are deleted with 11 commands and their two legacy features; the taxonomy and content-version gossip handlers only reject. `598bc0e`: the integrity committee co-sign path, its assurance projection, the unwired co-sign gossip handler, the unverified `integrity_set_anchor` setter that let any caller claim `anchored` in a signed credential, and the last legacy feature are deleted with 3 commands; achieved assurance is `local`, stored assurance and anchor columns are ignored at issuance, and sponsor roles requiring a reserved level fail as unavailable. The local snapshot commitment chain stays. `4393eef`: stale governance-queue comments corrected. No `legacy-*` feature or cfg site remains; remaining search matches are classified to D02 (compatibility and fake seed rows), D03 (retired tables) or historical/normative text. The five retired topics keep reject-only handlers until N01 removes them with relay and monitor configuration; verification closes with the M1 review |
| D02 | Verified | Reference inventory complete. `fcfc047`: the startup demo seed, its unlock backfill and demo-learner rebind, the demo media download, the plugin demo course, the `dev-seed` feature, the CLI and TUI seed commands and the fabricated-author catalog import are deleted; a fresh profile receives only labelled built-in data (`db::bundled`) — bundled taxonomy, synonyms, goal templates and question banks — and the course corpus moved to `demo-world/content/` as non-authoritative source data. `227c832`: the first-launch single-vault auto-migrator is deleted; startup now reports the unsupported pre-profile layout, leaves every entry untouched and onboards a fresh profile. `54633b1`: both keystores refuse the pre-HMAC salt file instead of reading it unverified, and an HMAC mismatch is reported as a wrong password rather than tampering. `5e6e08a`: iroh node secret keys are never written in the clear, a plaintext key file is refused instead of converted, and a failed content decrypt is an error rather than raw bytes (the deliberate public-plaintext evidence path stays). `04c2cd2`: version 2 is the only accepted course document format; signing, verification and enrolment refuse version 1 and its non-canonical signing path is gone. `676d3bd`: the retired single username `receipt` and every mirror and fallback that kept it alive are deleted, and a claim still carrying it is refused rather than counted at the receipted tier. `7ffc666`: the single-valued `account_role` is gone from the app, the column is no longer written, and onboarding requires roles and a birthdate so a minor cannot be created outside the guardian gate. `b9b32b2`: the pre-SQLCipher Sentinel profile import is deleted — the browser-storage key is erased rather than adopted — and the misnamed shortcuts cache key is renamed. The mobile-target check passed (`cargo check --target aarch64-apple-ios --features tutoring-video-ios`), closing the portable-keystore coverage gap. `0a42eb0`: the thirteen retired route redirects are deleted — `/unlock`, three superseded instructor authoring paths, `/skills/bootstrap`, `/governance`, the seven flattened `/dashboard/*` paths and the propose-prior path — so a stale deep link is no longer quietly rewritten to a route that may no longer mean what the old one did; the two remaining redirects are section entries, not compatibility. CORRECTION (M1 review): the row previously claimed such a link lands on a not-found page, which did not exist. `0bacbf7` adds the page and a catch-all route that the access guard still gates, so twelve of the retired paths now land there; `/skills/bootstrap` is captured by `/skills/:id` and shows that page's own error state for an unknown skill. Three comments describing behaviour the code no longer has were corrected in the same sweep: the IRT 1–5 difficulty column is the only parameter source there is rather than a legacy one, and the gossip validator never receives a database handle in a running node — the event loop runs `registry::check_message` as a profile-fenced executor job, so the fail-open identity branch is reachable only when the node started with no database at all. Two paths were classified rather than deleted: opening a profile already refuses an unencrypted SQLite file, and the unscoped `irl_review` rows belong to D03. `8fcb279`: the CIP-68 reputation mint is deleted — the asset-name and datum builders, the soulbound transaction builder, the Plutus mint helper and the freeze/load/project/tick recovery machinery — together with the items it orphaned: the soulbound and reputation-mint Plutus redeemer encoders and the ignored live preprod soulbound-update test, which spends a reference NFT only the deleted mint could have created. `snapshot_recovery` keeps only its two row readers, so a preserved row still lists and reads with its original hash and status, while requesting an anchor for one is refused by name rather than resuming a mint that will never be signed. `proficiency_to_index` moved verbatim to its single caller with its test, leaving the level filter that selects the verified evidence for a snapshot unchanged. D02 is source complete; `reputation_snapshot_inputs`, the asset-name columns and the `legacy_cip68` format value stay for the D03 baseline schema |
| D03 | Verified | Reference inventory complete: 127 `CREATE TABLE` statements minus the 16 tables created and later dropped leave 111 in the final schema, with one surviving view (`current_reputation_assertions`, seven production readers), three triggers and roughly 125 indexes, verified against `schema.rs` rather than inferred. The old scoring view `scoring_credentials` was already dropped at migration 94 and has no readers, so the baseline carries the surviving view rather than retiring it. Nine tables whose authority D01 and D02 deleted are still created and never dropped, and schema identity today is only a positional `(version, name)` prefix match of `_migrations` with `PRAGMA user_version` and `application_id` unused anywhere, so an old version-1 `initial_schema` database would silently satisfy a new baseline that reused that number and name. `16ee2fe`: the bundled seed wrote `bank_questions` while an attempt draws `assessment_items WHERE bank_id = ?`, and migration 072's backfill runs at migration time before the seed, so a fresh profile resolved its bundled bank, found no items and started an assessment with no questions and no error; the seed now writes assessment items and item skills directly, keeping prompt and options public and the key private, with tests that every bank fills its draw and an item withholds its key. The defect predates the rebuild - the seed deleted in `fcfc047` had the same shape - and the tests that missed it asserted row counts and foreign-key resolution, which cannot observe whether a question can be served. `07c8544`: one migration now creates the whole schema. It is not a textual concatenation of the 94 it replaces - those were replayed into a database, `sqlite_master` was dumped, and the dead tables and columns were removed from that dump, so the baseline is what the old chain actually produced rather than a reading of its SQL. Parity was then checked object by object: identical table sets, zero column mismatches across all 93 surviving tables in type, nullability, default and primary key, identical foreign keys, 105 of 105 indexes with byte-identical definitions, three triggers and the one surviving view. Eighteen tables and six columns are deliberately absent, every one of them having lost the code that gave it authority in D01 or D02, which turns those retirements structural: the plugin attestation test no longer inserts a forged row to prove it is ignored, it asserts the table cannot exist, and the governance and Sentinel gossip tests no longer set up a committee row because none can be held. The 22 migration-era schema tests are replaced by invariants of the schema itself - forbidden tables absent, retired columns absent, the objects the runtime reads present, foreign keys valid, initialisation idempotent, a failed baseline leaving nothing behind, and a database from the old chain refused rather than adopted. The integration suite under `src-tauri/tests` was outside the original reader survey and its guardian tests seeded the dropped `account_role`; the full workspace gate caught it, `cargo test --lib` never would have. `3476522`: the schema family is stamped in the SQLite file header and validated before any normal query. `application_id` carries a value derived from the family name and `user_version` the baseline epoch, so a foreign or future database is identified before a table is read and cannot be mistaken for this one by having been migrated to the same version number. The decision table fails closed in every case: an unstamped file with no tables is stamped and built; an unstamped file that already holds tables or migration history is refused as predating identity; a foreign application id is refused; an epoch ahead of this build is refused as written by a newer Alexandria; an epoch behind it is refused because no upgrade path exists yet; and a header disagreeing with the `_schema_identity` row is refused. No refusal converts or destroys: each names what was found and what was expected, offers moving the file or opening it with the build that wrote it, and says nothing was changed or deleted, with a test asserting that no refusal ever directs deletion of a file the operator has not examined. Validation sits at the head of `run_migrations_from_connection`, the one function both entry points reach, so the plan's requirement that the app and CLI share initialisation and validation holds by construction; the CLI's duplicate `_migrations` DDL is gone, and a test now builds a database through each entry point and compares every `sqlite_master` row and both identity pragmas rather than assuming the shared runner keeps them equal. D03 is source complete; verification closes with the M1 review alongside D01 and D02 |
...
| Documentation | Current through T03 | Active architecture, protocol, schema, VC migration, verifier README/vectors, skills docs, and root README describe migration 094, accepted/pending/rejected verification, trust classification, the manual endorsement workflow, the T03 action-to-policy matrix, verified scoring inputs with fingerprinted caches, and the retired issuer exclusion |

M1 review (2026-09-16): every recorded claim for T04, D01, D02 and D03 was audited against the current code by asking two questions — is it true, and would a test fail if it stopped being true. T04, D01, D02 and D03 are verified. The review found and fixed a dangling foreign key the baseline introduced (`opinion_withdrawals` referenced the dropped `governance_daos`), a false claim that stale deep links reached a not-found page when none existed, a ledger entry recording a superseded `application_id`, and a mobile keystore test that had never run and never reached its assertion. It wired the independent Node verifier into CI, added the not-found page, and added exact-boundary tests for all eight untrusted JSON limit sets with their values pinned independently of the constants. M1 is not complete: T02 item 7, authenticated addressed endorsement delivery, depends on M2 transport. Remaining known gaps are recorded in the ledger, including `docs/database-schema.md`, which still describes the 94-migration chain.

T03 is verified: derived skill states, reputation rows, snapshot evidence and
the talent-index reader share verified scoring inputs and revalidate cached
values by input fingerprint, and the credential proof panel no longer presents
a local revocation check as current remote status. T04 is source complete;
verification needs the cloud consumer to pass the same vectors in C01. D01 is
next. The preprod profile pins no qualification policies yet, so opinion
posting is refused with an actionable explanation until reviewed demo policies
are pinned.
...
3. Verify normalized name, nonce, issuer against configured issuers, signature, lifetime, original claimant binding, and release state before caching. Cache exact signed bytes and expiry; no indefinite trust from a JSON DID field. Parse DHT username claims, relay receipts and lookup responses with the verifier's strict bounded JSON parser under explicit limits; T04 hands these existing boundaries to this package.
...
2. Treat gossip as a notification that state may have changed. Reuse existing authenticated fetch/sync methods to recover missed state. Where no recovery exists, add a narrow versioned request/response path with shared limits and authority checks; do not invent a global raw-table sync endpoint. Set explicit request and response size maxima on every retained request-response codec and bound decoded collections: `vc-fetch`, `profile-fetch` and `graph-fetch` still use libp2p's 1 MiB request and 10 MiB response CBOR defaults. T04 hands these existing fetch boundaries to this package.
...
5. Review URL retrieval for uploaded job descriptions, OIDC discovery/JWKS where configurable, webhooks and presentation/holder fetches. Use approved HTTPS destinations, connect-time address checks, bounded DNS/redirect/body behavior and no proxy bypass where the destination is untrusted. Provider allowlists do not justify accepting arbitrary private redirect targets. Parse presentation envelopes and holder responses with the verifier's strict bounded JSON parser under explicit limits; T04 hands these existing boundaries to this package.
...
4. Pair only with authenticated proof of the same subject and an explicit short-lived one-time pairing code. Bound request size, pages, cursor lifetime and outstanding work; verify before advancing cursors. Sealed device-sync and guardian activity payloads are currently whole-table dumps with no size bound. Define and test their byte, page and collection limits, and parse opened payloads with the verifier's strict bounded JSON parser; T04 hands these boundaries to this package.
