...
| T04 | In progress | Strict bounded JSON parser in the verifier; network profile, credential file/bundle/peer import, offline verification, guardian link and course document boundaries parse under explicit limits with exact-boundary and one-over tests (in review); gossip payload validation, cross-language limit vectors and course-endorsement fixtures remain |
...
a local revocation check as current remote status. T04 is in progress. The
preprod profile pins no qualification policies yet, so opinion posting is
refused with an actionable explanation until reviewed demo policies are pinned.
