...
- **Decision needed — tenant IdP source of truth:** Cloud's `idp_connections` and `idp_login_domains` own per-tenant OIDC configuration and sealed secrets today. Recommended first step: one-way provisioning from Cloud into Keycloak Organizations through the Admin API (secrets duplicated into Keycloak). Moving console sign-in to Keycloak is a later option.
