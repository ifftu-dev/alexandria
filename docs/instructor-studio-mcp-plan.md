...
- Cloud lockfile: rustls 0.23.43 → 0.23.45 for RUSTSEC-2026-0285; `cargo audit` clean. The app workspace lockfile still carries the advisory.
...
3. **MCP Release 1 state (2026-09-17):** M0–M7 complete on both branches; M8 in progress. Both servers pass pinned-schema conformance, Inspector 2.6.0 `--strict` with every tool called, and Claude Code plus Codex CLI end to end (Cloud including each client's own OAuth sign-in through a pre-registered client). Remaining for M8: a staging deployment, to be approved, for ChatGPT and Claude on the web, which also exercises a hosted confidential registration for the first time. Under review: Cloud branch `codex/cloud-mcp-authz` as ifftu-dev/alexandria-cloud#1, app branch `codex/instructor-studio-mcp` as a pull request on ifftu-dev/alexandria. Merge both before staging so it deploys code `main` has. The app branch carries one commit from the local `main` that has not been pushed (`d4a655d`, the interview assistant); merge it with a merge commit, not squash or rebase, so that commit keeps its identity when local `main` is pushed. Open risks: Keycloak CIMD (keycloak#51236, canary in `scripts/mcp/e2e.py`); Keycloak 26.8 changes organisation-domain routing; the 2,146 organisations earlier test runs left in the Cloud development database were deleted on 2026-09-17 with their sync rows (suites now run against their own database via `scripts/test.sh`); `cargo audit` on the app workspace reports RUSTSEC-2026-0285 (rustls 0.23.43), fixed on the Cloud branch only.
