...
3. **M0 stdio conformance ledger complete** (see ledger). The remaining M0 decision is Cloud OAuth provider selection from the Desktop MCP plan, required before any Cloud private tool (M4). M1's offline verifier is independently usable now. Do not claim full M2–M8 conformance or Cloud completion from the narrow draft integration.
...
| C24 | Streamable HTTP, header validation, Origin, OAuth | transports, authorization | Cloud (M4) | — | Not applicable to stdio; pending provider selection |
