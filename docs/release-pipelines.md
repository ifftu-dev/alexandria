...
- `Release (Desktop)` / `Release (Mobile)`: tag-push publish workflows for real releases.
...
- Publish workflows only run from immutable tags, which keeps validation and publishing from competing with each other.
...
2. Let `Release (Desktop)` publish desktop bundles and updater metadata.
3. Let `Release (Mobile)` publish Android artifacts and, when enabled, signed iOS exports.
