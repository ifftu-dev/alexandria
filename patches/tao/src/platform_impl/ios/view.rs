...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // `autorelease_return`, not `as_ptr`. This is a UIKit delegate method
      // whose selector does not begin with new/alloc/copy, so the caller does
      // not own what comes back and UIKit retains it itself — later, in
      // -[UIApplication _connectUISceneFromFBSScene:transitionContext:].
      //
      // `Retained::as_ptr` only borrows, so the `Retained` still owned the
      // configuration and released it as this function returned. objc2 claims
      // the autoreleased return of `configurationWithName_sessionRole` with
      // objc_retainAutoreleasedReturnValue, which takes it out of the pool, so
      // that release was the last one: UIKit then retained freed memory and the
      // app died in objc_retain with EXC_BAD_ACCESS before its first frame.
      // Handing the +1 to the autorelease pool keeps the object alive across
      // the return and lets UIKit take its own reference.
