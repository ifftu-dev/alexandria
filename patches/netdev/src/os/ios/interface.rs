...
// LOCAL PATCH (alexandria): The upstream iOS `interfaces()` augments the
// getifaddrs list with Network.framework, SystemConfiguration and BSD-route
// data. On a real iPhone one of those augmentation paths hands back a `None`
// that is unwrapped downstream, aborting the process (SIGABRT) the moment the
// iroh/netwatch p2p node enumerates interfaces after wallet unlock. The
// SystemConfiguration / route APIs it reaches for are macOS-only anyway (they
// are linked as NULL stubs in the app on iOS), so drop the augmentation and
// return the plain getifaddrs list refined only by static, side-effect-free
...
#[allow(dead_code)]
fn interfaces_upstream() -> Vec<Interface> {
    let mut ifaces: Vec<Interface> = unix_interfaces();
