...
///
/// The own-member guard has one exception. SwiftPM statically vendors this
/// crate's own Swift runtime (`SwiftRs.o`) into *every* package archive, and
/// the Rust side of swift-rs unconditionally references three of its @_cdecl
/// exports — `retain_object`, `release_object`, `string_from_bytes`. Leaving
/// those internal in every archive means nothing in the whole link defines
/// them, so any consumer with a Swift package fails with "Undefined symbols".
/// Promoting them in one archive would work only by accident of which package
/// happens to be present, so they are promoted in all of them and immediately
/// weakened: weak definitions coalesce instead of colliding, which is exactly
/// the "one of these identical copies wins" semantics this needs.
/// @_cdecl exports of this crate's Swift runtime, vendored into every archive.
...
fn globalize_cdecl_symbols(archive: &std::path::Path, package_name: &str) {
    ...
    let mut in_runtime_member = false;
    ...
    for line in String::from_utf8_lossy(&nm.stdout).lines() {
        ...
        *occurrences.entry(name.to_string()).or_insert(0) += 1;
        if kind == "t" && in_runtime_member && SWIFT_RS_RUNTIME_EXPORTS.contains(&name) {
            if !weak_candidates.iter().any(|s| s == name) {
                weak_candidates.push(name.to_string());
            }
            continue;
        }
    ...
    let syms: Vec<String> = candidates
        ...
        .collect();
    // The runtime exports keep the uniqueness guard too: more than one copy in
    // a single archive is the duplicate-atom crash this is all trying to avoid.
    let weak_syms: Vec<String> = weak_candidates
        .into_iter()
        .filter(|s| occurrences.get(s).copied().unwrap_or(0) <= 1)
        .collect();
    if syms.is_empty() && weak_syms.is_empty() {
        return;
    }
    ...
    for s in &weak_syms {
        // Order matters to llvm-objcopy only in that both apply; the result is
        // a weak global, which is what lets every archive carry a copy.
        cmd.arg(format!("--globalize-symbol={s}"));
